...
import { isPatch, isConfidenceValid, isOperationsValid } from "../types/patch.js";
 ...
 */
async function generateDiff(patch) {
    let content;
    let found = true;
    try {
    ...
    const operations = patch.patch.operations.map((op, i) => {
...
function printDiff(diff) {
    ...
    diff.operations.forEach(op => {
        if (op.found) {
            process.stdout.write(`  ${GREEN}✓${RESET} Operasyon #${op.index}\n` +
            ...
            process.stdout.write(`  ${RED}✗${RESET} Operasyon #${op.index} — ${YELLOW}search metni bulunamadı${RESET}\n` +
...
export async function runDryRun(filePath, opts = {}) {
    ...
    if (result.status !== "PASS") {
        if (opts.json) {
            ...
            process.stderr.write(`${prefix} — ${CYAN}${filePath}${RESET}\n\n` +
    ...
    if (policyResult.status === "DENY" || policyResult.status === "BLOCK") {
        ...
        if (opts.json) {
            ...
            process.stderr.write(`${RED}${BOLD}✗ ${policyResult.status}${RESET} — ${CYAN}${filePath}${RESET}\n\n` +
    ...
    if (policyResult.status === "ASK_HUMAN") {
        ...
        if (opts.json) {
            ...
            process.stdout.write(`${YELLOW}${BOLD}⚠ ASK_HUMAN${RESET} — ${CYAN}${filePath}${RESET}\n\n` +
                `  Kaynak  : ${policyResult.source}\n` +
                `  Sebep   : ${redirect}\n\n`);
        }
        return 2;
