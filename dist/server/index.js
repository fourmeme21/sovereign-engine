...
function broadcast(msg) {
    ...
    clients.forEach(ws => { if (ws.readyState === WebSocket.OPEN)
        ws.send(raw); });
}
// ─── JWT TOKEN ÜRETIMI ───────────────────────────────────────
function issueToken(patch) {
    ...
    return jwt.sign(payload, JWT_SECRET, { algorithm: 'HS256', expiresIn: 30 });
}
function evaluatePatch(patch) {
    ...
    const amount = patch.payload?.['amount'];
...
async function mcpProxy(subpath, body) {
    if (!MCP_URL)
        return { ok: false, error: 'MCP_NOT_CONFIGURED', hint: 'Set MCP_URL env variable to your local mcp-server ngrok URL' };
    try {
...
wss.on('connection', (ws) => {
    clients.add(ws);
    ws.send(JSON.stringify({ type: 'init', decisions: decisionStore }));
    ws.on('message', (raw) => {
        try {
