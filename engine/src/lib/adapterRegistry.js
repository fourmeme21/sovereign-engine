 ...
 */
export async function loadRegistry(userId, tier) {
    ...
    for (const adapter of adapters) {
        for (const category of adapter.categories) {
            ...
            if (categoryMap.has(category)) {
                console.warn(`[AdapterRegistry] Kategori çakışması: "${category}" ` +
 ...
 */
export function matchCategory(category, registry) {
 ...
 */
export async function registerAdapter(userId, tier, adapterName, adapterCode, categories) {
    const limit = ADAPTER_LIMITS[tier] ?? 1;
    ...
    if ((count ?? 0) >= limit) {
        throw new Error(`Tier limiti aşıldı: ${tier} planı en fazla ${limit} adapter destekler. ` +
