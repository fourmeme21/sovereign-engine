 ...
 */
async function fetchAndCache(projectId) {
    try {
        ...
        const entry = {
            core_doc: data.core_doc ?? '',
            ai_agent_doc: data.ai_agent_doc ?? '',
            cached_at: Date.now(),
            refreshing: false,
        };
 ...
 */
function readSessionIndex(localMemoryPath) {
    try {
...
// TOKEN SAYACI
// ---------------------------------------------------------------------------
function getCounterKey(userId, projectId) {
 ...
 */
function addTokens(userId, projectId, inputTokens, outputTokens) {
 ...
 */
function isThresholdReached(userId, projectId) {
    const key = getCounterKey(userId, projectId);
 ...
 */
function resetCounter(userId, projectId) {
    const key = getCounterKey(userId, projectId);
 ...
 */
export function getTokenCount(userId, projectId) {
    const key = getCounterKey(userId, projectId);
...
// ---------------------------------------------------------------------------
async function buildInjectionResult(userId, projectId, localMemoryPath) {
    const empty = {
        injected: false,
        system_suffix: '',
        tokens_reset: false,
    ...
    resetCounter(userId, projectId);
    console.log(`[contextInjector] Enjeksiyon tetiklendi — ` +
 ...
 */
export async function checkAndInjectProactive(userId, projectId, localMemoryPath) {
    const empty = {
        injected: false,
        system_suffix: '',
        tokens_reset: false,
    ...
    if (!projectId)
        return empty;
    if (!isThresholdReached(userId, projectId))
        return empty;
 ...
 */
export async function checkAndInject(userId, projectId, localMemoryPath, inputTokens, outputTokens) {
    const empty = {
        injected: false,
        system_suffix: '',
        tokens_reset: false,
 ...
 */
export async function preloadProjectCache() {
    try {
        const { data, error } = await supabase
            .from('user_projects')
            .select('id')
...
// ---------------------------------------------------------------------------
export function evictProjectCache(projectId) {
