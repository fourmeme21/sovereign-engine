...
// ---------------------------------------------------------------------------
function getFileInstruction(fileName) {
    const instructions = {
        'CORE.md': `
...
// ---------------------------------------------------------------------------
async function extractFileSummary(claude, fileName, content) {
    try {
...
// ---------------------------------------------------------------------------
function packIntoSessions(files, masterPlan) {
    const sessions = [];
...
// ---------------------------------------------------------------------------
function buildSystemPrompt(projectName, masterPlan, priorContext, isFirstSession) {
...
// ---------------------------------------------------------------------------
function extractFileContent(response, fileName) {
    const pattern = new RegExp(`<FILE\\s+name=["']?${fileName.replace('.', '\\.')}["']?>([\\s\\S]*?)</FILE>`, 'i');
...
// ---------------------------------------------------------------------------
async function markFileStatus(projectId, fileName, status, errorMsg) {
    await supabase
...
// ---------------------------------------------------------------------------
function extractCategoriesFromAdapter(content) {
    ...
    if (xmlMatch?.[1]) {
        try {
            ...
            const cats = parsed.filter((c) => typeof c === 'string' && catPattern.test(c));
            if (cats.length > 0)
                return cats;
        }
...
// ---------------------------------------------------------------------------
async function upsertAdapter(userId, adapterName, adapterCode, categories) {
    ...
    if (existing) {
        const { error } = await supabase
            ...
            .eq('id', existing.id);
        if (error)
            throw new Error(`adapter güncelleme hatası: ${error.message}`);
        return;
    }
    const { error } = await supabase
        .from('user_adapters')
    ...
    if (error)
        throw new Error(`adapter kayıt hatası: ${error.message}`);
}
// ---------------------------------------------------------------------------
// YARDIMCI: Supabase'e dosya kaydet
// ---------------------------------------------------------------------------
async function saveToSupabase(projectId, fileName, content) {
    if (fileName === 'CORE.md') {
        const { error } = await supabase
            .from('user_projects')
            ...
            .eq('id', projectId);
        if (error)
            throw new Error(`core_doc yazılamadı: ${error.message}`);
        return;
    }
    if (fileName === 'AI_AGENT.md') {
        const { error } = await supabase
            ...
            .eq('id', projectId);
        if (error)
            throw new Error(`ai_agent_doc yazılamadı: ${error.message}`);
        return;
    }
...
async function generateSingleFile(claude, projectId, file, systemPrompt, retryCount = 0) {
...
export async function runGeneration(opts) {
    const { projectId, userId, projectName, masterPlan, completedFiles = [], bestEffort = true, } = opts;
    console.log(`[generationEngine] Başladı — ${projectId} (${projectName}) | mode: ${bestEffort ? 'best-effort' : 'strict'}`);
    const claude = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });
    const allFiles = DEFAULT_FILE_PLAN.map(f => ({
        ...f,
    ...
    for (let si = 0; si < sessions.length; si++) {
        ...
        let sessionSystemPrompt = buildSystemPrompt(projectName, masterPlan, priorContext, isFirstSession);
        console.log(`[generationEngine] Oturum ${si + 1}/${sessions.length}: ` +
        ...
        for (const file of session.files) {
            try {
                ...
                if (cumulativeTokens >= CONTEXT_REFRESH_THRESHOLD) {
                    console.log(`[generationEngine] ⚠️ Context yenileniyor — ` +
                        `kümülatif token: ${cumulativeTokens} (eşik: ${CONTEXT_REFRESH_THRESHOLD})`);
                    sessionSystemPrompt = buildSystemPrompt(projectName, masterPlan, priorContext, false);
                    cumulativeTokens = 0;
                }
                const allDefaultDone = DEFAULT_FILE_PLAN.every(f => successFiles.includes(f.fileName));
                if (allDefaultDone) {
                    await supabase
                        .from('user_projects')
    ...
    console.log(`[generationEngine] ${finalStatus === 'completed' ? '✅' : '⚠️'} Bitti — ` +
...
// ---------------------------------------------------------------------------
export async function resumeGeneration(projectId, userId, projectName, masterPlan) {
