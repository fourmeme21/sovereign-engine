...
import { ASTParser, ASTChunk } from "./astParser.js";
import {
  SemanticDiff, SymbolDelta, RiskFactors, ChangeIntent
} from "./semanticDiff.types.js";
...
export function generateSemanticDiff(
  ...
  afterContent: string,
  options: { commitSha?: string; branch?: string; traceId?: string } = {}
): SemanticDiff {
  ...
  const symbolsAdded:    SymbolDelta[] = [];
  const symbolsRemoved:  SymbolDelta[] = [];
  const symbolsModified: SymbolDelta[] = [];
  ...
  const riskFactors = computeRiskFactors(
    filePath, symbolsRemoved, symbolsModified, importsAdded, linesAdded + linesRemoved
  );
...
function toSymbolDelta(chunk: ASTChunk, intent: ChangeIntent): SymbolDelta {
  return {
    name: chunk.name,
    type: chunk.type,
    intent,
...
function computeRiskFactors(
  filePath: string,
  symbolsRemoved: SymbolDelta[],
  symbolsModified: SymbolDelta[],
  importsAdded: string[],
...
function computeRiskScore(
  ...
  if (factors.touches_auth)     score += 4;
  if (factors.touches_payment)  score += 4;
  if (factors.touches_security) score += 3;

  if (factors.deletes_symbols)  score += 3;
  if (factors.modifies_exports) score += 2;
  if (factors.large_change)     score += 2;
  if (factors.adds_imports)     score += 1;

  if (removed.length > 3)  score += 2;
  if (modified.length > 5) score += 1;
