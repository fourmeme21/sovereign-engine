...
import { checkIntegrity, openSession, checkpoint, touchActivity, closeSession, } from '../lib/sessionManager.js';
...
function validateChatBody(body) {
    ...
    if (b['device_id'] !== undefined && b['device_id'] !== null) {
        ...
        if (!DEVICE_UUID_REGEX.test(b['device_id'])) {
...
function quickRiskFilter(message) {
    const denyPatterns = [
...
function buildRiskPrompt(userMessage, assistantReply) {
...
async function analyzeRiskWithClaude(userMessage, assistantReply, claudeClient) {
    try {
        ...
        const validVerdicts = ['PERMIT', 'ASK_HUMAN', 'DENY'];
        const validPolicies = ['POL-CHAT-001', 'POL-CHAT-002', 'POL-CHAT-003', 'POL-CHAT-004', 'POL-CHAT-DENY'];
        ...
        console.warn('[analyzeRiskWithClaude] Claude analiz hatası — fail-closed ASK_HUMAN:', err instanceof Error ? err.message : String(err));
        return { score: 6, verdict: 'ASK_HUMAN', policy: 'POL-CHAT-002', reason: 'Risk analizi tamamlanamadı — güvenli tarafta kalınıyor.' };
    }
}
async function scoreChatRisk(userMessage, assistantReply, claudeClient) {
...
async function acquireDeviceLock(projectId, userId, deviceId) {
    try {
        ...
        console.warn('[acquireDeviceLock] Supabase hatası — kilit atlandı:', err instanceof Error ? err.message : String(err));
        return { acquired: true };
    }
}
async function releaseDeviceLock(projectId, userId, deviceId) {
    try {
        ...
        console.warn('[releaseDeviceLock] Supabase hatası:', err instanceof Error ? err.message : String(err));
    }
}
// ─── TB-14: TOKEN SAYACI ──────────────────────────────────────
...
async function incrementTokenCount(projectId, userId, amount) {
    try {
        ...
        console.warn('[incrementTokenCount] Supabase hatası:', err instanceof Error ? err.message : String(err));
        return 0;
    }
}
async function resetTokenCount(projectId, userId) {
    try {
        ...
        console.warn('[resetTokenCount] Supabase hatası:', err instanceof Error ? err.message : String(err));
    }
}
async function fetchCoreDocsRow(projectId, userId) {
...
function buildCoreDocsSuffix(row) {
    return [
        '---',
        '## PROJE CORE DOKÜMANI',
        row.core_doc,
...
async function injectCoreDocsIfNeeded(baseSystemPrompt, projectId, userId, isFirstMessage) {
    if (!projectId) {
    ...
    try {
        ...
        console.warn('[injectCoreDocsIfNeeded] Supabase hatası — inject atlandı:', err instanceof Error ? err.message : String(err));
        return { systemPrompt: baseSystemPrompt, injected: false, tokenReset: false };
    }
}
// ─── /chat YARDIMCILARI ───────────────────────────────────────
async function runSessionSetup(userId, projectId, localMemoryPath, isFirstMessage) {
    ...
    if (projectId)
        touchActivity(userId, projectId, localMemoryPath);
    return null;
}
async function buildSystemPromptWithInjection(userId, projectId, localMemoryPath, isFirstMessage) {
...
async function callClaudeChat(messages, maxTokens, systemPrompt) {
    const response = await claude.messages.create({
        model: AI_MODEL,
...
async function applyCodeQualityGuard(userText, reply) {
    ...
    try {
        ...
        console.warn('[applyCodeQualityGuard] Guard hatası — orijinal reply korunuyor:', err instanceof Error ? err.message : String(err));
        return { reply, qualityMeta: null };
    }
}
// ─── /apply YARDIMCILARI ──────────────────────────────────────
function buildExecutionContext(userId, tier, sessionId) {
    return {
        actor_id: userId,
        actor_role: tier,
...
function checkForbiddenPatterns(adapterCode) {
...
async function buildAdapterSandbox() {
    const { createContext, Script } = await import('vm');
    const sandboxExports = {};
    const sandbox = createContext({
        exports: sandboxExports,
        console: {
            log: (...args) => console.log('[adapter]', ...args),
            warn: (...args) => console.warn('[adapter]', ...args),
            error: (...args) => console.error('[adapter]', ...args),
        },
...
async function runAdapterExecution(adapterCode, actionName, params, context) {
    ...
    const AdapterClass = (sandboxExports['default'] ??
        Object.values(sandboxExports)[0]);
...
router.post('/chat', async (req, res) => {
    ...
    const { messages, max_tokens = 1024, project_id = null, local_memory_path = null, is_first_message = false, session_action = null, device_id = null, } = validation.data;
    // SSC-4-EXEMPT: express.d.ts augmentation req.user.id — Karar #54
    const userId = req.user?.id ?? 'anonymous'; // SSC-4-EXEMPT: Karar #54 express.d.ts
    ...
    try {
        ...
        const { systemPrompt, proactiveInjection, coreInjected, tokenReset } = await buildSystemPromptWithInjection(userId, project_id, local_memory_path, is_first_message);
        ...
        console.error(`[aiProxy/chat] Anthropic error [${traceId}]:`, err instanceof Error ? err.message : String(err));
...
router.post('/apply', async (req, res) => {
    ...
    try {
        ...
        const tier = req.userTier ?? 'free'; // SSC-4-EXEMPT: Karar #54
        ...
        let actionResult;
        try {
            actionResult = await runAdapterExecution(match.adapter.adapter_code, decision.payload.action_name, decision.payload.params ?? {}, context);
        }
        catch (execErr) {
        ...
        // SSC-5: iç hata mesajı dışa sızdırılmıyor; trace_id ile izlenebilirlik sağlanıyor
        console.error(`[aiProxy/apply] Beklenmeyen hata [${traceId}]:`, err instanceof Error ? err.message : String(err));
...
router.post('/session/close', async (req, res) => {
    ...
    try {
        ...
        console.error('[aiProxy/session/close] Hata:', err instanceof Error ? err.message : String(err));
        if (device_id && typeof device_id === 'string') {
            await releaseDeviceLock(project_id, userId, device_id);
        }
