...
router.post("/upload", async (req, res) => {
    ...
    try {
        const result = await processFileUpload(project_id, file_name, content, commit_sha, branch, undefined, trace.trace_id);
        let policyResult = {
            verdict: "PERMIT",
...
router.post("/query", async (req, res) => {
    ...
    try {
        ...
        if (error)
            throw error;
        if (data?.length) {
            await supabase.rpc("increment_reference_counts", { chunk_ids: data.map((r) => r.id) });
        }
        res.json({
...
router.get("/health/:project_id", async (req, res) => {
    ...
    const summary = (data || []).reduce((acc, row) => {
        if (!acc[row.memory_type])
            acc[row.memory_type] = { count: 0, avgConfidence: 0, avgFreshness: 0 };
...
async function generateContinuityBriefing(projectId, lastSessionAt) {
    const now = new Date();
    ...
    if (gapDays > 0 && lastSessionAt) {
        const { data: commits } = await supabase
            ...
            .limit(10);
        if (commits?.length) {
            gitSummary = `${commits.length} commit: ${commits.map((c) => c.semantic_diff_summary || c.message).join("; ")}`;
        }
    }
    ...
    const activeZone = zones?.[0]?.directory || "Belirlenemedi";
    ...
    return {
        ...
        unresolved_threads: (threads || []).map((t) => ({ id: t.id, summary: t.content, confidence: t.confidence, file: t.source_path })),
        active_zone: activeZone,
        architectural_drift_detected: false,
        recommended_context_files: zones?.map((z) => z.directory) || [],
        last_decision_ids: (decisions || []).map((d) => d.id),
        suggested_focus: suggestedFocus,
        recent_risky_changes: (recentHighRisk || []).map((d) => ({
            file: d.file_path,
            risk_score: d.risk_score,
            summary: d.semantic_summary || buildQuickSummary(d),
            when: d.generated_at,
        })),
        recent_decisions: (recentDecisionEvents || []).map((e) => ({
...
function buildQuickSummary(d) {
    const parts = [];
    if (d.symbols_added?.length)
        parts.push(`${d.symbols_added.length} eklendi`);
    if (d.symbols_removed?.length)
        parts.push(`${d.symbols_removed.length} silindi`);
    if (d.symbols_modified?.length)
        parts.push(`${d.symbols_modified.length} değişti`);
