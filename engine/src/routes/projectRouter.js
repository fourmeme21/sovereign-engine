...
async function checkProjectLimit(userId, tier) {
...
// ---------------------------------------------------------------------------
function buildDefaultFiles(projectId) {
    const files = [
...
async function insertProject(params) {
    ...
    return { project: data, conflictError: false, triggerError: false };
}
// ---------------------------------------------------------------------------
// ROUTER
// ---------------------------------------------------------------------------
...
router.post('/create', async (req, res) => {
    ...
    try {
        ...
        if (!project)
            throw new Error('Insert sonucu boş döndü.');
    ...
    try {
        ...
        if (req.query.resume === 'true' && p.gen_status !== 'completed') {
            const { data: mp } = await supabase
                ...
                .eq('id', projectId)
                .single();
            const masterPlan = mp?.master_plan ?? '';
            if (masterPlan) {
                setImmediate(() => {
...
router.post('/:id/file', async (req, res) => {
    ...
    const validTargets = ['supabase', 'local_warm', 'local_hot'];
    ...
    try {
        ...
        if (existing) {
            ...
            const { data: maxOrder } = await supabase
                ...
                .limit(1)
                .single();
            const nextOrder = (maxOrder?.file_order ?? 10) + 1;
...
// ═══════════════════════════════════════════════════════════════════════════
router.put('/:id/masterplan', async (req, res) => {
    const userId = req.user.id;
    const projectId = req.params.id;
    // SSC-3: UUID formatı doğrula
    if (!isValidUuid(projectId)) {
    ...
    try {
        ...
        res.json({
            ...
            revision_instructions: {
                ...
                instruction: 'Claude\'a yeni master planı ver. ' +
