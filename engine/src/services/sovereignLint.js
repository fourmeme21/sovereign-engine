...
function checkIntentComment(code) {
...
function checkFakeData(lines) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (/['"`]TODO:\s*PROD/i.test(line))
            continue; // işaretlenmiş, geçer
...
function checkTestExists(code) {
    const hasTest = /it\(|test\(|describe\(/.test(code) ||
...
// ─── ANA LINT FONKSİYONU ────────────────────────────────────────────────────
export function runSovereignLint(code, filename) {
    ...
    failedRules.forEach(r => { if (INTENTIONAL_WARN_RULES.has(r))
        failedRules.delete(r); });
...
function buildSummary(score, failedRules, warnRules, filename) {
...
//# sourceMappingURL=sovereignLint.js.map