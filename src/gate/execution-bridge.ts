...
export type GateBridgeStatus =
  | "SUCCESS"      // execution tamamlandı
  | "FAILED"       // execution başarısız + rolled_back
  | "DENIED"       // token doğrulama başarısız
  | "ERROR";       // binary crash / timeout / parse hatası
 ...
 */
export async function callExecutionGate(
  input: ExecutionGateInput
): Promise<GateBridgeResult> {
  ...
  return new Promise((resolve) => {
    ...
    let gate: ReturnType<typeof spawn>;
    try {
    ...
    // Process kapandı
    gate.on("close", (code: number | null) => {
      clearTimeout(timer);
      ...
      try {
        parsed = JSON.parse(raw) as ExecutionResult;
      } catch (e) {
        resolve({
          status:      "ERROR",
      ...
      const status: GateBridgeStatus = parsed.success
        ? "SUCCESS"
        : parsed.rolled_back
          ? "FAILED"
          : "DENIED";
 ...
 */
export async function executionGateHealthCheck(): Promise<boolean> {
  return new Promise((resolve) => {
    let gate: ReturnType<typeof spawn>;
    try {
