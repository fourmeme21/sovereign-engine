...
import { supabase } from '../lib/supabaseClient'; // kendi yolunla değiştir
import { authMiddleware } from '../middleware/authMiddleware'; // kendi yolunla
...
// ─── D.4 WEBHOOK ─────────────────────────────────────────────
// ⚠️ Bu route raw body istiyor — app.ts'de bu route için express.json() ÖNCE gelememez
router.post(
  '/webhook',
      ...
      switch (eventType) {
        case 'subscription.active': {
          ...
          await supabase
            ...
            .or(`email.eq.${data.customer?.email}`);
          break;
        }
