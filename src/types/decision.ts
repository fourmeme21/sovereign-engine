 ...
 */
export type Intent =
  | "READ_DATA"       // Veri okuma          — LOW risk
  | "WRITE_DATA"      // Kayıt oluşturma     — MEDIUM risk
  | "EXECUTE_ACTION"  // İş akışı tetikleme  — HIGH risk
  | "TRIGGER_EVENT"   // Olay / bildirim     — MEDIUM risk
  | "MODIFY_STATE";   // Durum / config      — CRITICAL risk (zorunlu)
...
export type ImmutableDecisionStatus =
  | "COMPLETED"
  | "REJECTED"
  | "BLOCKED";
...
export function isImmutableStatus(status: DecisionStatus): status is ImmutableDecisionStatus {
  return status === "COMPLETED" || status === "REJECTED" || status === "BLOCKED";
}

/** MODIFY_STATE intent'i için risk seviyesi CRITICAL mi? */
export function isModifyStateValid(decision: Decision): boolean {
...
export function isAssumedStateValid(decision: Decision): boolean {
  if (decision.payload.assumed_state !== undefined) {
    return (
      decision.intent === "EXECUTE_ACTION" ||
