...
export type SchemaErrorCode =
  ...
  | "INVALID_VERSION"     // schema_version uyumsuz
  | "INVALID_FORMAT"      // Regex veya tip uyumsuzluğu
  | "INVALID_TYPE"        // Beklenen tip yanlış
  | "NON_POSITIVE_VALUE"; // session_number ≤ 0
...
function err(field: string, code: SchemaErrorCode, message: string): SchemaError {
...
export function validateSchema(raw: unknown): SchemaValidationResult {
  ...
  if (d["schema_version"] === undefined || d["schema_version"] === null) {
    ...
    errors.push(err(
      "schema_version",
      "INVALID_VERSION",
      `Desteklenmeyen schema_version: "${String(d["schema_version"])}" — beklenen "1.0"`
    ));
  }
  ...
  if (typeof d["category"] !== "string" || d["category"].length === 0) {
    ...
    errors.push(err(
      "category",
      "INVALID_FORMAT",
      `category yalnızca büyük harf ve alt çizgi içerebilir: "${d["category"]}"`
    ));
  }
  ...
  if (!isObject(d["payload"])) {
    ...
    if (typeof payload["action_name"] !== "string" || payload["action_name"].length === 0) {
      ...
      errors.push(err(
        "payload.action_name",
        "INVALID_FORMAT",
        `action_name yalnızca küçük harf ve alt çizgi içerebilir: "${payload["action_name"]}"`
      ));
    }
  ...
  if (!isObject(d["metadata"])) {
    ...
    if (typeof meta["session_number"] !== "number") {
      ...
      errors.push(err(
        ...
        "NON_POSITIVE_VALUE",
        `session_number 0 veya negatif olamaz: ${meta["session_number"]}`
      ));
    }
 ...
 */
export function castToDecision(raw: unknown): Decision {
